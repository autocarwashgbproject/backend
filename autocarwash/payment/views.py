...
from autocarwash.secrets import BANK_KEY_SECRET, BANK_ACCOUNT_ID, SITE_URL
...
def card_update(user_card, payment):
    ...
    # user_card.bank = payment.payment_method.card.issuer_name
    # TODO для тестов убираем, так как не приходит название банка
    ...
    def post(self, request, *args, **kwargs):
        ...
        if car_id and isinstance(is_regular_pay, bool):
            ...
            if subscription_date.exists():
                ...
                is_subscribe = SubscriptionCar.is_subscribe(
            ...
            if is_subscribe:
                ...
                if user_card.payment_method_saved:
                    ...
                    if check_pay.paid:
                        car = Car.objects.get(id=car_id)
                        SubscriptionCar.objects.create(
                            reg_num=car, subscription=subscription
                            )
                        return Response({
                            'ok': True,
                            'pay': True,
                    ...
                    payment = yandex_pay.pay_without_payment_id(
                        order=order, value=value
                        )
                    order_update(order=order, payment=payment)
                    user_card.payment_id = payment.payment_method.id
                    user_card.save()
                    # TODO если человек не нажмет после яндекс оплаты
                    # вернутся в магазин, то оплату не проверим,
                    # соответственно не будет подписки у клиента и оплаты
                    # проверенной, в связи с этим надо запускать
                    # ResultCreateView самостоятельно по опредленным ид,
                    # которые в течении последних 5 минут запросили оплату,
                    # запускать каждые 3 минуты (то есть по всем новым
                    # OrderBankUsers)
                    return Response({
                        'ok': True,
        ...
        # если нету car_id, is_regular_pay
            ...
            return Response({
                'ok': False,
                'error_code': 404,
                'description': "We can't see car_id or is_regular_pay " + \
                    'or user_card_id or all of them'
                    })
...
class ResultCreateView(View):
    def get(self, request, *args, **kwargs):
        ...
        if order.is_paid:
            ...
            if check_pay == False:
                context = {
                    'pay': f"Вы еще не провели оплату, вернутся на страницу" + \
                    "оплаты: https://money.yandex.ru/payments/external" + \
                    f"/confirmation?orderId={order.card_bank_users.payment_id}"
                    }
            ...
            if check_pay.paid:
                SubscriptionCar.objects.create(
                    reg_num=order.car, subscription=order.subscription
                    )
                context = {
                    'pay': "Оплачено"
                    }
